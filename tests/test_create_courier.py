...
class TestCreateCourier:
    ...
    def test_create_courier_with_valid_data_status_code(self, courier_data, cleanup_courier):
    ...
    @allure.step('Отправляем POST запрос на создание курьера с валидными данными')
    def test_create_courier_with_valid_data_login_message(self, courier_data, cleanup_courier):
        response = requests.post(Urls.CREATE_COURIER, json=courier_data)
        courier_id = helpers.get_courier_id(courier_data['login'], courier_data['password']) 
        cleanup_courier(courier_id)
    ...
    def test_create_courier_without_login_status_code(self,courier_without_login):
    ...
    @allure.step('Отправляем POST запрос на создание курьера без логина')
    def test_create_courier_without_login_message(self,courier_without_login):
        response = requests.post(Urls.CREATE_COURIER, json=courier_without_login)
    ...
    def test_create_courier_without_password_status_code(self,courier_without_password):
    ...
    @allure.step('Отправляем POST запрос на создание курьера без пароля')
    def test_create_courier_without_password_message(self,courier_without_password):
        response = requests.post(Urls.CREATE_COURIER, json=courier_without_password)
        assert response.json()["message"] == "Недостаточно данных для создания учетной записи"